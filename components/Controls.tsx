...
import {
  ExpandIcon, CollapseIcon, PauseIcon, PlayIcon, RepeatIcon, SendIcon
} from './icons';
...
export const Controls: React.FC<ControlsProps> = ({
  ...
  const isPlaying = status === 'DRAWING';
  ...
  const ControlButton: React.FC<{onClick: () => void, children: React.ReactNode, disabled?: boolean}> = ({ onClick, children, disabled }) => (
    <button
      onClick={onClick}
      disabled={disabled}
      className="p-2 w-10 h-10 flex items-center justify-center rounded-lg bg-black text-neutral-400 hover:text-white transition-all duration-200 transform hover:scale-110 active:scale-95 disabled:opacity-0 disabled:pointer-events-none"
    >
      {children}
    </button>
  );
  ...
  return (
    <div className="absolute bottom-4 left-4 right-4 md:bottom-6 md:left-1/2 md:-translate-x-1/2 md:right-auto w-full max-w-3xl md:w-auto md:min-w-[700px]">
      <div className="bg-neutral-900 rounded-2xl p-2 sm:p-3 transition-all duration-300 hover:shadow-[0_0_20px_0px_rgba(255,255,255,0.05)]">

        {/* Top Row: Animated Prompts + Control Buttons */}
        <div className="flex items-center justify-between h-14">
          <div className="flex-1 mr-4 pl-2">
            {showIdleState ? (
              <AnimatedPrompts onPromptClick={handlePromptClick} isPlaying={!isPaused} />
            ) : (
              <div className="h-8 flex items-center">
                <span className="text-lg text-neutral-400">{explanation}</span>
              </div>
            )}
          </div>

          <div className="flex items-center gap-1 sm:gap-2">
            <ControlButton onClick={onTogglePause} disabled={pauseControlDisabled}>
              {isPaused ? <PlayIcon /> : <PauseIcon />}
            </ControlButton>
            <ControlButton onClick={onRepeat} disabled={repeatControlDisabled}>
              <RepeatIcon />
            </ControlButton>
        ...
        {/* Expandable Section */}
        <div className={`transition-all duration-500 ease-in-out grid ${isExpanded ? 'grid-rows-[1fr] opacity-100 pt-2' : 'grid-rows-[0fr] opacity-0'}`}>
