...
export const Controls: React.FC<ControlsProps> = ({
  ...
  const ControlButton: React.FC<{onClick: () => void, children: React.ReactNode, className?: string}> = ({ onClick, children, className }) => (
    <button
      onClick={onClick}
      className={`p-3 w-12 h-12 flex items-center justify-center rounded-full bg-black text-neutral-400 hover:text-white transition-all duration-200 transform hover:scale-110 active:scale-95 border border-neutral-800 hover:border-neutral-700 relative z-10 ${className || ''}`}
    >
      {children}
    </button>
  );
  ...
  return (
    <div className="absolute bottom-4 left-4 right-4 md:bottom-6 md:left-1/2 md:-translate-x-1/2 md:right-auto w-full max-w-4xl md:w-auto md:min-w-[700px] z-20">
