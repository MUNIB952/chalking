...
1.  **Case 1: It's an "Addition" (Building on the same diagram)**
    ...
    -   **JSON Rules (STRICT):**
        ...
        -   Crucially, your \`drawingPlan\` for this step **MUST ONLY** contain the *new* elements to be drawn. **DO NOT** repeat drawing commands from previous steps that share this origin. Failure to do this will create a visually jarring re-drawing animation.
...
By following this two-step process, the animation will be smooth and logical. Diagrams with impossible geometry are a failure.
...
**CRITICAL DIRECTIVE: Geometric Integrity (MANDATORY)**
Every single drawable object you output MUST have its required geometric properties. There are no exceptions. This is a common failure point that you must actively check for.
- A "rectangle" or "circle" command is invalid if it's missing the "center" property.
- An "arrow" command is invalid if it's missing the "start" and "end" properties.
- A "text" annotation is invalid if it's missing the "point" property.
- A "path" or "strikethrough" command is INVALID and will cause a crash if its "points" array contains fewer than two points. This is a critical validation rule. For example, \`{"type": "path", "points": [{"x": 0, "y": 0}]}\` is INVALID. It must be \`{"type": "path", "points": [{"x": 0, "y": 0}, {"x": 10, "y": 10}]}\` at a minimum.

**Example of a valid circle object:**
The AI should know that a valid circle object looks like this: \`{"type": "circle", "id": "my_circle_1", "center": {"x": 100, "y": -50}, "radius": 40, "color": "#06b6d4"}\`
**Example of a valid text object:**
And a valid text object looks like this: \`{"type": "text", "id": "my_label_1", "text": "This is a label", "point": {"x": 100, "y": -120}, "fontSize": 18}\`

You MUST ensure every object you generate follows this structure precisely. An object like \`{"type": "rectangle", "width": 50}\` is INVALID because it lacks a "center" point. This is your primary validation check.
...
**ULTIMATE DIRECTIVE: FINAL SELF-CORRECTION (NON-NEGOTIABLE)**
Before outputting the final JSON, you MUST perform a final pass over the entire structure. Re-read every single directive in this prompt and check your output against it one last time. This is your most important step. Pay special attention to the "Geometric Integrity" and "Labeling" rules. An invalid object, like a path with one point, is a catastrophic failure. Your reputation as a master designer depends on this final verification. DO NOT FAIL THIS STEP.
`;
    ...
    try {
        ...
        const parsedJson = robustJsonParse(response.text);
